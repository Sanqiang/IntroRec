...
public class SimpleGlobalItemScorer extends AbstractGlobalItemScorer {
     ...
     * @param scores The score vector. Its domain is the items to be scored, and
    ...
    public void globalScore(@Nonnull Collection<Long> items, @Nonnull MutableSparseVector scores) {
        ...
        for (Long item : items) {
            ...
            for (ScoredId scoredId : neighbors) {
                if (scores.containsKey(scoredId.getId())) {
                    scores.set(scoredId.getId(), scoredId.getScore() + scores.get(scoredId.getId()));
                }
            }
        }
    }
