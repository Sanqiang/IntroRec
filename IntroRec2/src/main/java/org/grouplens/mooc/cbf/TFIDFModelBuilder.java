...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
     ...
     *
     * @param dao The item-tag DAO. This is where the builder will get access to
     * items and their tags.
     * <p>{@link Transient} means that the provider promises that the DAO is no
     * longer needed once the object is built (that is, the model will not
     * contain a reference to the DAO). This allows LensKit to configure your
     * recommender components properly. It's up to you to keep this promise.</p>
     */
    @Inject
    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            for (String tag : tags) {
                ...
                work.set(tagId, 1 + work.get(tagId));
            }
        ...
        for (VectorEntry e : docFreq.fast()) {
            ...
            int countDoc = 0;
            for (long item : items) {
                if (itemVectors.get(item).containsKey(e.getKey())) {
                    ++countDoc;
                }
            }
            docFreq.set(e.getKey(), countDoc);
        }
        ...
        for (Map.Entry<Long, MutableSparseVector> entry : itemVectors.entrySet()) {
            ...
            for (VectorEntry e : tv) {
                long tagId = e.getKey();
                double tf = e.getValue();// tagCount;
                double idf = Math.log((double)items.size() / docFreq.get(tagId));
                tv.set(tagId, tf * idf);
            }
            ...
            double base = tv.norm();
            for (VectorEntry e : tv) {
                tv.set(e.getKey(), e.getValue() / base);
            }
