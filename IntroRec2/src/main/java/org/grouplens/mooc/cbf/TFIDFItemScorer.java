...
public class TFIDFItemScorer extends AbstractItemScorer {
     ...
     *
     * @param dao The user-event DAO, so we can fetch a user's ratings when
     ...
     * @param output The output vector. The contract of this method is that the
     * caller creates a vector whose possible keys are all items that should be
     * scored; this method fills in the scores.
     */
    @Override
    public void score(long user, @Nonnull MutableSparseVector output) {
        ...
        for (VectorEntry e : output.fast(VectorEntry.State.EITHER)) {
            ...
            double score = iv.dot(userVector);
            double base = iv.norm() * userVector.norm();
            output.set(e.getKey(), score / base);
        }
    }
    ...
    private SparseVector makeUserVector(long user) {
        ...
        for (Rating r : userRatings) {
            ...
            if (p != null && p.getValue() >= 0) {
                mean += r.getPreference().getValue();
            }
        }
        mean /= userRatings.size();
        ...
        for (Rating r : userRatings) {
            ...
            if (p != null && p.getValue() >= 3.5) {
                ...
                SparseVector tags = model.getItemVector(itemId);
                for (VectorEntry e : tags) {
                    long tagId = e.getKey();
                    if (!profile.containsKey(tagId)) {
                        profile.set(tagId, 0);
                    }
                    profile.set(e.getKey(), e.getValue() + profile.get(tagId));
                }
            }
            */ 
            //part 2
            ...
            if (p != null && p.getValue() >= 0) {
                ...
                for (VectorEntry e : tags) {
                    ...
                    profile.set(e.getKey(), e.getValue() * (p.getValue() - mean) + profile.get(tagId));
                }
            }
        }
