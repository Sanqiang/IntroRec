...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector scores) {
        ...
        RealMatrix userMatrix = MatrixUtils.createRealMatrix(1, model.getItemFeatureMatrix().getRowDimension()); //1*uk
        for (VectorEntry entry : userVector) {
            int item_ind = model.getItemIndMapping().getIndex(entry.getKey());
            userMatrix.setEntry(0, item_ind, entry.getValue());
        }*/
        
        RealMatrix userMatrix = model.getUserVector(user);
        RealMatrix weights = model.getFeatureWeights();//uk*uk
        RealMatrix itemMatrix = model.getItemFeatureMatrix();//uk * i
        RealMatrix finalScore = userMatrix.multiply(weights).multiply(itemMatrix.transpose()); //1*i
        for (VectorEntry e : scores.fast(VectorEntry.State.EITHER)) {
            long item = e.getKey();
            // TODO Set the scores
            double score = finalScore.getEntry(0, model.getItemIndMapping().getIndex(item)) + baselineScorer.score(user, item);;
