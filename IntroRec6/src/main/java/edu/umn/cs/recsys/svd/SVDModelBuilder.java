...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        int UserNum = userMapping.size(), ItemNum = itemMapping.size();
        RealMatrix U = svd.getU().getSubMatrix(0, UserNum-1, 0, featureCount-1);
        RealMatrix V = svd.getV().getSubMatrix(0, ItemNum-1, 0, featureCount-1);
        RealMatrix E = svd.getS().getSubMatrix(0, featureCount-1, 0, featureCount-1);

        SVDModel model = new SVDModel(userMapping, itemMapping, U, V, E);
    ...
    /**
     * Build a rating matrix from the rating data. Each user's ratings are first
        ...
        try {
            for (UserHistory<Event> user : users) {
                ...
                for (VectorEntry rating : ratings) {
                    matrix.setEntry(u, itemMapping.getIndex(rating.getKey()), rating.getValue());
                }
            }
        } finally {
            users.close();
